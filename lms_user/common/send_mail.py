...
def send_email(request,user, text):
    try:
        ...
        elif text == 'Reminder to leave issuer':
            msg['Subject'] = 'LMS | Reminder for pending leave'
            body = render_to_string('lms_user/reminder_email_to_issuer.html', {
                'user': user,
                'domain': current_site.domain,
                'protocol':"https"
            })  
        elif text == 'Reminder to user':
            msg['Subject'] = 'LMS | Reminder for pending leave'
            body = render_to_string('lms_user/reminder_email_to_user.html', {
                'user': user,
                'domain': current_site.domain,
                'protocol':"https"
            }) 
